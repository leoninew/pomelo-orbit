...
func TestHandleMarksRunFaultedWhenStageFails(t *testing.T) {
	store := &fakeStore{
		run:  orbit.PipelineRun{Id: "run-1", RepositoryId: "repo-1", SnapshotId: "snapshot-1", VariablesSnapshot: `{}`},
		repo: orbit.Repository{Id: "repo-1", Code: "repo"},
		snapshot: orbit.PipelineSnapshot{Id: "snapshot-1", StagesSnapshot: `[
	...
	err := handler.Handle(context.Background(), task.Task{PayloadJSON: `{"pipeline_run_id":"run-1"}`})
	if err != nil {
		t.Fatalf("Handle returned error: %v", err)
	}
	if store.runStatus != orbit.WorkStatusFaulted {
	...
	for _, stageRun := range store.stageRuns {
		if stageRun.Status == orbit.WorkStatusCanceled {
			canceled = true
		}
	}
	if !canceled {
...
func TestHandleExecutesPipelineRun(t *testing.T) {
	store := &fakeStore{
		...
		repo:     orbit.Repository{Id: "repo-1", Code: "repo"},
		snapshot: orbit.PipelineSnapshot{Id: "snapshot-1", StagesSnapshot: `[{"id":"stage-1","name":"build","image":"alpine","script":"echo ok"}]`},
	}
	handler := Handler{
		store:  store,
	...
	if store.runStatus != orbit.WorkStatusRanToCompletion {
		t.Fatalf("unexpected final run status: %s", store.runStatus)
	}
	if len(store.stageRuns) != 1 {
		t.Fatalf("expected 1 stage run, got %d", len(store.stageRuns))
	}
	if store.stageRuns[0].Status != orbit.WorkStatusRanToCompletion {
...
type fakeStore struct {
	mu         sync.Mutex
	run        orbit.PipelineRun
	repo       orbit.Repository
	snapshot   orbit.PipelineSnapshot
	stageRuns  []orbit.StageRun
	runStarted bool
	runStatus  string
}

func (s *fakeStore) PipelineRun(ctx context.Context, id string) (orbit.PipelineRun, error) {
	return s.run, nil
}

func (s *fakeStore) Repository(ctx context.Context, id string) (orbit.Repository, error) {
	return s.repo, nil
}

func (s *fakeStore) PipelineSnapshot(ctx context.Context, id string) (orbit.PipelineSnapshot, error) {
...
func (s *fakeStore) InsertStageRun(ctx context.Context, stage orbit.StageRun) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *fakeStore) UpdateStageRun(ctx context.Context, stage orbit.StageRun) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *fakeStore) InsertArtifact(ctx context.Context, projectId *string, run orbit.PipelineRun, stageName string, artifact orbit.ArtifactConfig, path string) error {
	return nil
}
