...
func (s Server) importApplication(w http.ResponseWriter, r *http.Request) {
	...
	app := orbit.Application{Id: orbit.NewId(), ProjectId: &projectId, Name: req.Name, Code: req.Code, ImagePullPolicy: req.ImagePullPolicy, RouteManaged: req.RouteManaged, Status: orbit.ApplicationStatusUndeployed}
	files := make([]orbit.ApplicationConfigFile, 0, len(req.ConfigFiles))
	for _, file := range req.ConfigFiles {
		files = append(files, orbit.ApplicationConfigFile{Id: orbit.NewId(), ApplicationId: app.Id, Path: file.Path, Content: file.Content})
	}
	serviceConfigs := make([]orbit.ApplicationServiceConfig, 0, len(req.ServiceConfigs))
	for _, item := range req.ServiceConfigs {
		...
		serviceConfigs = append(serviceConfigs, orbit.ApplicationServiceConfig{Id: orbit.NewId(), ApplicationId: app.Id, ServiceName: item.ServiceName, Image: image, Environment: environment, Volumes: volumes})
	}
	routes := make([]orbit.ApplicationRoute, 0, len(req.Routes))
	for _, item := range req.Routes {
		routes = append(routes, orbit.ApplicationRoute{Id: orbit.NewId(), ApplicationId: app.Id, ServiceName: item.ServiceName, Domain: item.Domain, Port: item.Port})
	}
...
func (s Server) createApplicationFile(w http.ResponseWriter, r *http.Request) {
	...
	file := orbit.ApplicationConfigFile{Id: orbit.NewId(), ApplicationId: app.Id, Path: req.Path, Content: req.Content}
...
func (s Server) deployApplication(w http.ResponseWriter, r *http.Request) {
	...
	payload, _ := json.Marshal(map[string]string{"application_id": app.Id, "deployment_id": deployment.Id})
	if err := s.tasks.Enqueue(r.Context(), orbit.NewId(), status.TaskTypeCDApplicationDeploy, string(payload), s.defaultMaxAttempts); err != nil {
...
func (s Server) stopApplication(w http.ResponseWriter, r *http.Request) {
	...
	if output, err := runApplicationCommand(r, appDir, cmd...); err != nil {
		_ = s.store.CompleteDeployment(r.Context(), deployment.Id, orbit.WorkStatusFaulted, outputOrError(output, err))
		writeJSON(w, http.StatusInternalServerError, map[string]string{"detail": "Failed to stop application"})
		return
	}
	if err := s.store.MarkApplicationStatus(r.Context(), app.Id, orbit.ApplicationStatusUndeployed); err != nil {
	...
	if err := s.store.CompleteDeployment(r.Context(), deployment.Id, orbit.WorkStatusRanToCompletion, ""); err != nil {
...
func (s Server) restartApplication(w http.ResponseWriter, r *http.Request) {
	...
	payload, _ := json.Marshal(map[string]string{"application_id": app.Id, "deployment_id": deployment.Id})
	if err := s.tasks.Enqueue(r.Context(), orbit.NewId(), status.TaskTypeCDApplicationRestart, string(payload), s.defaultMaxAttempts); err != nil {
...
func (s Server) createApplicationRoute(w http.ResponseWriter, r *http.Request) {
	...
	route := orbit.ApplicationRoute{Id: orbit.NewId(), ApplicationId: app.Id, ServiceName: req.ServiceName, Domain: req.Domain, Port: req.Port}
...
func (s Server) updateApplicationServiceConfig(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			config = orbit.ApplicationServiceConfig{Id: orbit.NewId(), ApplicationId: app.Id, ServiceName: serviceName}
		} else {
...
func configFileResponse(file orbit.ApplicationConfigFile) configFileResp {
	return configFileResp{Id: file.Id, Path: file.Path, CreatedAt: formatTime(file.CreatedAt)}
}

func applicationRouteResponse(route orbit.ApplicationRoute) applicationRouteResp {
...
func (s Server) newApplicationDeployment(app orbit.Application, operationType string) orbit.Deployment {
	return orbit.Deployment{Id: orbit.NewId(), ProjectId: app.ProjectId, ApplicationId: &app.Id, ApplicationName: app.Name, OperationType: operationType, TriggerType: "manual", Status: orbit.WorkStatusWaitingToRun, IsRollback: false}
}

func (s Server) loadApplicationFileForCurrentUser(w http.ResponseWriter, r *http.Request) (orbit.ApplicationConfigFile, bool) {
	app, ok := s.loadApplicationForCurrentUser(w, r)
	if !ok {
		return orbit.ApplicationConfigFile{}, false
	}
...
func (s Server) loadApplicationRouteForCurrentUser(w http.ResponseWriter, r *http.Request) (orbit.ApplicationRoute, bool) {
	app, ok := s.loadApplicationForCurrentUser(w, r)
	if !ok {
		return orbit.ApplicationRoute{}, false
	}
...
func (s Server) loadApplicationCompose(w http.ResponseWriter, r *http.Request) (orbit.Application, orbit.ApplicationConfigFile, bool) {
	app, ok := s.loadApplicationForCurrentUser(w, r)
	if !ok {
		return orbit.Application{}, orbit.ApplicationConfigFile{}, false
	}
	files, err := s.store.ConfigFiles(r.Context(), app.Id)
	if err != nil {
		s.logger.Error("load application config files failed", "application_id", app.Id, "error", err)
...
func (s Server) renderApplicationCompose(w http.ResponseWriter, r *http.Request, app orbit.Application, compose orbit.ApplicationConfigFile) (string, bool) {
...
func (s Server) composeServices(w http.ResponseWriter, r *http.Request, app orbit.Application, compose orbit.ApplicationConfigFile) (map[string]any, bool) {
	content := compose.Content
	if strings.HasSuffix(compose.Path, ".jinja") {
...
func (s Server) applicationServiceConfigView(app orbit.Application, serviceName string, raw any, config *orbit.ApplicationServiceConfig) applicationServiceConfigResp {
...
func applyApplicationServiceConfigs(compose string, configs []orbit.ApplicationServiceConfig) string {
	if len(configs) == 0 {
		return compose
	}
	var data map[string]any
...
func injectApplicationRouteLabels(compose string, routes []orbit.ApplicationRoute, letsEncrypt bool) (string, error) {
	var data map[string]any
	if err := yaml.Unmarshal([]byte(compose), &data); err != nil {
		return "", err
	}
