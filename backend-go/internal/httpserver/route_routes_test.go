...
func TestRouteTraefikConfig(t *testing.T) {
	certPEM := "cert"
	route := orbit.Route{Name: "web", Domain: "web.example.test", PathPrefix: "/api", TargetURL: "http://host.docker.internal:8080", Enabled: true, HTTPSEnabled: true, CertPEM: &certPEM, CertType: certTypeManual}
