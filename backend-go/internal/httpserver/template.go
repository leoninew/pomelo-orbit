...
func (s Server) listPipelineTemplates(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, newPaginatedResp(orbit.Page[pipelineTemplateResp]{Items: responses, Total: items.Total, Page: items.Page, PerPage: items.PerPage}))
}
...
func (s Server) createPipelineTemplate(w http.ResponseWriter, r *http.Request) {
	...
	template := orbit.PipelineTemplate{Id: orbit.NewId(), ProjectId: &projectId, Name: req.Name, Description: req.Description, VariableDeclarations: variables, Version: 1}
...
func (s Server) duplicatePipelineTemplate(w http.ResponseWriter, r *http.Request) {
	...
	duplicated := orbit.PipelineTemplate{Id: orbit.NewId(), ProjectId: template.ProjectId, Name: name, Description: template.Description, VariableDeclarations: template.VariableDeclarations, Version: 1}
	stages := make([]orbit.PipelineTemplateStage, 0, len(existingStages))
	for _, stage := range existingStages {
		stage.Id = orbit.NewId()
...
func (s Server) loadPipelineTemplateForCurrentUser(w http.ResponseWriter, r *http.Request) (orbit.PipelineTemplate, bool) {
	current, ok := s.currentUser(w, r)
	if !ok {
		return orbit.PipelineTemplate{}, false
	}
...
func (s Server) writePipelineTemplateDetail(w http.ResponseWriter, r *http.Request, template orbit.PipelineTemplate, status int) {
...
func (s Server) pipelineTemplateResponse(w http.ResponseWriter, r *http.Request, template orbit.PipelineTemplate) (pipelineTemplateResp, bool) {
...
func (s Server) pipelineTemplateStagesResponse(w http.ResponseWriter, r *http.Request, projectId string, orchestration []stageOrchestrationResp) ([]buildStageResp, bool) {
	...
	stageMap := make(map[string]orbit.BuildStage, len(stages))
...
func (s Server) applyPipelineTemplateUpdateReq(w http.ResponseWriter, r *http.Request, template *orbit.PipelineTemplate, req map[string]json.RawMessage) ([]orbit.PipelineTemplateStage, bool, bool) {
	projectId := pipelineTemplateProjectId(*template)
	versionChanged := false
	orchestrationProvided := false
	stages := []orbit.PipelineTemplateStage(nil)
...
func (s Server) loadBuildStagesForOrchestration(w http.ResponseWriter, r *http.Request, projectId string, orchestration []stageOrchestrationResp) ([]orbit.BuildStage, bool) {
	stageIds := make([]string, 0, len(orchestration))
...
func pipelineTemplateStageRows(w http.ResponseWriter, templateId string, orchestration []stageOrchestrationResp, stages []orbit.BuildStage) ([]orbit.PipelineTemplateStage, bool) {
	stageMap := make(map[string]orbit.BuildStage, len(stages))
	for _, stage := range stages {
		stageMap[stage.Id] = stage
	}
	rows := make([]orbit.PipelineTemplateStage, 0, len(orchestration))
	for _, item := range orchestration {
		stageId := strings.TrimSpace(item.StageId)
		...
		rows = append(rows, orbit.PipelineTemplateStage{Id: orbit.NewId(), TemplateId: templateId, StageId: stage.Id, StageName: stage.Name, StageVersion: stage.Version, DependsOn: dependsOn, SortOrder: item.SortOrder})
	}
	return rows, true
}
...
func pipelineTemplateProjectId(template orbit.PipelineTemplate) string {
...
func resolveTemplateVariablesFromResponses(stages []buildStageResp, custom []map[string]any) []map[string]any {
	converted := make([]orbit.BuildStage, 0, len(stages))
	for _, stage := range stages {
		artifacts, _ := json.Marshal(stage.Artifacts)
		artifactString := string(artifacts)
		converted = append(converted, orbit.BuildStage{Name: stage.Name, Script: stage.Script, Artifacts: &artifactString})
	}
	return resolveTemplateVariables(converted, custom)
}

func resolveTemplateVariables(stages []orbit.BuildStage, custom []map[string]any) []map[string]any {
