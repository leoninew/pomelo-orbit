...
func (s Store) PipelineRun(ctx context.Context, id string) (PipelineRun, error) {
	var run PipelineRun
	err := s.db.GetContext(ctx, &run, `SELECT id, project_id, repository_id, repository_name, snapshot_id, template_id,
		template_name, template_version, trigger, trigger_ref, variables_snapshot, status, retry_of,
		started_at, finished_at, error_message, created_at FROM pipeline_run WHERE id = ?`, id)
	if err != nil {
...
func (s Store) Repository(ctx context.Context, id string) (Repository, error) {
	...
	err := s.db.GetContext(ctx, &repo, `SELECT id, project_id, name, code, repository_url, git_credential_id,
		variable_overrides, default_branch FROM repository WHERE id = ?`, id)
	if err != nil {
...
func (s Store) PipelineSnapshot(ctx context.Context, id string) (PipelineSnapshot, error) {
	...
	err := s.db.GetContext(ctx, &snapshot, `SELECT id, project_id, template_id, version, stages_snapshot,
		variables_snapshot FROM pipeline_snapshot WHERE id = ?`, id)
	if err != nil {
...
func (s Store) Credential(ctx context.Context, id string) (Credential, error) {
	...
	err := s.db.GetContext(ctx, &credential, `SELECT id, project_id, name, type, encrypted_data FROM credential WHERE id = ?`, id)
	if err != nil {
...
func (s Store) MarkPipelineRunRunning(ctx context.Context, id string) error {
	_, err := s.db.ExecContext(ctx, `UPDATE pipeline_run SET status = ?, started_at = datetime('now'), error_message = NULL WHERE id = ?`, WorkStatusRunning, id)
	if err != nil {
...
func (s Store) CompletePipelineRun(ctx context.Context, id string, status string, message string) error {
	_, err := s.db.ExecContext(ctx, `UPDATE pipeline_run SET status = ?, finished_at = datetime('now'), error_message = NULLIF(?, '') WHERE id = ?`, status, message, id)
	if err != nil {
...
func (s Store) InsertArtifact(ctx context.Context, projectId *string, run PipelineRun, stageName string, artifact ArtifactConfig, path string) error {
	_, err := s.db.ExecContext(ctx, `INSERT INTO artifact (id, project_id, pipeline_run_id, repository_id, repository_name, template_id, template_name, stage_name, type, name, path, created_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))`, NewId(), projectId, run.Id, run.RepositoryId, run.RepositoryName, run.TemplateId, run.TemplateName, stageName, artifact.Type, artifact.Name, path)
	if err != nil {
...
func (s Store) Application(ctx context.Context, id string) (Application, error) {
	var app Application
	err := s.db.GetContext(ctx, &app, `SELECT id, project_id, name, code, image_pull_policy, status, route_managed FROM application WHERE id = ?`, id)
	if err != nil {
...
func (s Store) Deployment(ctx context.Context, id string) (Deployment, error) {
	...
	err := s.db.GetContext(ctx, &deployment, `SELECT id, project_id, application_id, operation_type, status, started_at, finished_at, error_message FROM deployment WHERE id = ?`, id)
	if err != nil {
...
func (s Store) ConfigFiles(ctx context.Context, applicationId string) ([]ApplicationConfigFile, error) {
	...
	err := s.db.SelectContext(ctx, &files, `SELECT id, application_id, path, content FROM application_config_file WHERE application_id = ? ORDER BY path`, applicationId)
	if err != nil {
...
func (s Store) ServiceConfigs(ctx context.Context, applicationId string) ([]ApplicationServiceConfig, error) {
	...
	err := s.db.SelectContext(ctx, &configs, `SELECT id, application_id, service_name, image, environment, volumes FROM application_service WHERE application_id = ?`, applicationId)
	if err != nil {
...
func (s Store) Routes(ctx context.Context, applicationId string) ([]ApplicationRoute, error) {
	...
	err := s.db.SelectContext(ctx, &routes, `SELECT id, application_id, service_name, domain, port FROM application_route WHERE application_id = ?`, applicationId)
	if err != nil {
...
func (s Store) MarkApplicationStatus(ctx context.Context, id string, status string) error {
	_, err := s.db.ExecContext(ctx, `UPDATE application SET status = ?, updated_at = datetime('now') WHERE id = ?`, status, id)
	if err != nil {
...
func (s Store) MarkDeploymentRunning(ctx context.Context, id string) error {
	_, err := s.db.ExecContext(ctx, `UPDATE deployment SET status = ?, started_at = datetime('now'), error_message = NULL WHERE id = ?`, WorkStatusRunning, id)
	if err != nil {
...
func (s Store) CompleteDeployment(ctx context.Context, id string, status string, message string) error {
	_, err := s.db.ExecContext(ctx, `UPDATE deployment SET status = ?, finished_at = datetime('now'), duration_ms = CAST((julianday(datetime('now')) - julianday(started_at)) * 86400000 AS INTEGER), error_message = NULLIF(?, '') WHERE id = ?`, status, message, id)
	if err != nil {
