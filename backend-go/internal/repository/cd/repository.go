...
func (r Repository) ListApplications(ctx context.Context, projectId *string, page int, perPage int, search string) (repository.Page[repository.Application], error) {
	...
	if err := r.db.GetContext(ctx, &total, `SELECT COUNT(*) FROM application`+where, args...); err != nil {
		return repository.Page[repository.Application]{}, fmt.Errorf("count applications: %w", err)
	}
	args = append(args, perPage, (page-1)*perPage)
	var items []repository.Application
	...
	if err != nil {
		return repository.Page[repository.Application]{}, fmt.Errorf("list applications: %w", err)
	}
	return repository.Page[repository.Application]{Items: items, Total: total, Page: page, PerPage: perPage}, nil
}

func (r Repository) Application(ctx context.Context, id string) (repository.Application, error) {
	...
	if err != nil {
		return repository.Application{}, fmt.Errorf("load application %s: %w", id, err)
	}
	return app, nil
}

func (r Repository) ApplicationByName(ctx context.Context, name string) (repository.Application, error) {
	var app repository.Application
	...
	if err != nil {
		return repository.Application{}, fmt.Errorf("load application by name %s: %w", name, err)
	}
	return app, nil
}

func (r Repository) ApplicationByCode(ctx context.Context, code string) (repository.Application, error) {
	var app repository.Application
	...
	if err != nil {
		return repository.Application{}, fmt.Errorf("load application by code %s: %w", code, err)
	}
	return app, nil
}

func (r Repository) CreateApplication(ctx context.Context, app repository.Application) error {
...
func (r Repository) CreateApplicationBundle(ctx context.Context, app repository.Application, files []repository.ApplicationConfigFile, serviceConfigs []repository.ApplicationServiceConfig, routes []repository.ApplicationRoute) error {
...
func (r Repository) UpdateApplication(ctx context.Context, app repository.Application) error {
...
func (r Repository) ConfigFiles(ctx context.Context, applicationId string) ([]repository.ApplicationConfigFile, error) {
	var files []repository.ApplicationConfigFile
...
func (r Repository) ConfigFile(ctx context.Context, id string) (repository.ApplicationConfigFile, error) {
	var file repository.ApplicationConfigFile
	...
	if err != nil {
		return repository.ApplicationConfigFile{}, fmt.Errorf("load config file %s: %w", id, err)
	}
	return file, nil
}

func (r Repository) CreateConfigFile(ctx context.Context, file repository.ApplicationConfigFile) error {
...
func (r Repository) UpdateConfigFile(ctx context.Context, file repository.ApplicationConfigFile) error {
...
func (r Repository) ServiceConfigs(ctx context.Context, applicationId string) ([]repository.ApplicationServiceConfig, error) {
	var configs []repository.ApplicationServiceConfig
...
func (r Repository) ApplicationServiceConfig(ctx context.Context, applicationId string, serviceName string) (repository.ApplicationServiceConfig, error) {
	...
	if err != nil {
		return repository.ApplicationServiceConfig{}, fmt.Errorf("load application service config %s/%s: %w", applicationId, serviceName, err)
	}
	return config, nil
}

func (r Repository) UpsertApplicationServiceConfig(ctx context.Context, config repository.ApplicationServiceConfig) error {
	var count int
...
func (r Repository) Routes(ctx context.Context, applicationId string) ([]repository.ApplicationRoute, error) {
...
func (r Repository) ApplicationRoute(ctx context.Context, id string) (repository.ApplicationRoute, error) {
	var route repository.ApplicationRoute
	...
	if err != nil {
		return repository.ApplicationRoute{}, fmt.Errorf("load application route %s: %w", id, err)
	}
	return route, nil
}

func (r Repository) CreateApplicationRoute(ctx context.Context, route repository.ApplicationRoute) error {
...
func (r Repository) UpdateApplicationRoute(ctx context.Context, route repository.ApplicationRoute) error {
...
func (r Repository) ListRoutes(ctx context.Context, projectId string, page int, perPage int, search string) (repository.Page[repository.Route], error) {
	page, perPage = repository.NormalizePage(page, perPage)
	...
	if err := r.db.GetContext(ctx, &total, `SELECT COUNT(*) FROM route`+where, args...); err != nil {
		return repository.Page[repository.Route]{}, fmt.Errorf("count routes: %w", err)
	}
	args = append(args, perPage, (page-1)*perPage)
	var items []repository.Route
	...
	if err != nil {
		return repository.Page[repository.Route]{}, fmt.Errorf("list routes: %w", err)
	}
	return repository.Page[repository.Route]{Items: items, Total: total, Page: page, PerPage: perPage}, nil
}

func (r Repository) ListAllRoutes(ctx context.Context, projectId string) ([]repository.Route, error) {
	var items []repository.Route
	err := r.db.SelectContext(ctx, &items, `SELECT id, project_id, name, domain, path_prefix, target_url, enabled, https_enabled, cert_pem, cert_key, cert_type, created_at, updated_at
...
func (r Repository) Route(ctx context.Context, id string) (repository.Route, error) {
	var route repository.Route
	...
	if err != nil {
		return repository.Route{}, fmt.Errorf("load route %s: %w", id, err)
	}
	return route, nil
}

func (r Repository) RouteByDomain(ctx context.Context, domain string) (repository.Route, error) {
	var route repository.Route
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return repository.Route{}, sql.ErrNoRows
		}
		return repository.Route{}, fmt.Errorf("load route by domain %s: %w", domain, err)
	}
	return route, nil
}

func (r Repository) CreateRoute(ctx context.Context, route repository.Route) error {
...
func (r Repository) UpdateRoute(ctx context.Context, route repository.Route) error {
...
func (r Repository) CreateDeployment(ctx context.Context, deployment repository.Deployment) error {
...
func (r Repository) ListDeployments(ctx context.Context, projectId string, applicationId string, status string, search string, dateFrom *time.Time, dateTo *time.Time, page int, perPage int) (repository.Page[repository.Deployment], error) {
	page, perPage = repository.NormalizePage(page, perPage)
	...
	if err := r.db.GetContext(ctx, &total, `SELECT COUNT(*) FROM deployment`+where, args...); err != nil {
		return repository.Page[repository.Deployment]{}, fmt.Errorf("count deployments: %w", err)
	}
	args = append(args, perPage, (page-1)*perPage)
	var items []repository.Deployment
	...
	if err != nil {
		return repository.Page[repository.Deployment]{}, fmt.Errorf("list deployments: %w", err)
	}
	return repository.Page[repository.Deployment]{Items: items, Total: total, Page: page, PerPage: perPage}, nil
}

func (r Repository) Deployment(ctx context.Context, id string) (repository.Deployment, error) {
	...
	if err != nil {
		return repository.Deployment{}, fmt.Errorf("load deployment %s: %w", id, err)
	}
	return deployment, nil
}

func (r Repository) CancelDeployment(ctx context.Context, id string) error {
	_, err := r.db.ExecContext(ctx, fmt.Sprintf(`UPDATE deployment SET status = ?, finished_at = %s, duration_ms = %s, error_message = ? WHERE id = ?`, db.NowExpr(r.driver), db.DurationMillisExpr(r.driver, "started_at")), repository.WorkStatusCanceled, "Cancelled by user", id)
	if err != nil {
...
func (r Repository) MarkDeploymentRunning(ctx context.Context, id string) error {
	_, err := r.db.ExecContext(ctx, fmt.Sprintf(`UPDATE deployment SET status = ?, started_at = %s, error_message = NULL WHERE id = ?`, db.NowExpr(r.driver)), repository.WorkStatusRunning, id)
	if err != nil {
