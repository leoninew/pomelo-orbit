...
func (s Server) createBuildStage(w http.ResponseWriter, r *http.Request) {
	...
	stage := repository.BuildStage{Id: repository.NewId(), ProjectId: &projectId, Name: req.Name, Image: req.Image, Script: req.Script, Artifacts: artifacts, Description: req.Description, Version: 1}
...
func (s Server) loadBuildStageForCurrentUser(w http.ResponseWriter, r *http.Request) (repository.BuildStage, bool) {
	current, ok := s.currentUser(w, r)
	if !ok {
		return repository.BuildStage{}, false
	}
...
func (s Server) applyBuildStageUpdateReq(w http.ResponseWriter, r *http.Request, stage *repository.BuildStage, req map[string]json.RawMessage) bool {
	projectId := buildStageProjectId(*stage)
