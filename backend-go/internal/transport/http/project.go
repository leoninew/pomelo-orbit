...
func (s Server) createProject(w http.ResponseWriter, r *http.Request) {
	...
	now := time.Now().UTC()
	project := repository.Project{Id: repository.NewId(), Name: req.Name, Code: req.Code, IsActive: true, CreatedAt: now, UpdatedAt: now}
...
func (s Server) addProjectMember(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := s.userRepository.UserById(r.Context(), req.UserId); err != nil {
...
func (s Server) loadProjectForCurrentUser(w http.ResponseWriter, r *http.Request) (repository.Project, bool) {
	current, ok := s.currentUser(w, r)
	if !ok {
...
func (s Server) loadProjectForUser(w http.ResponseWriter, r *http.Request, userId string) (repository.Project, bool) {
...
func projectResponse(project repository.Project) projectResp {
	return projectResp{
		Id:        project.Id,
		Name:      project.Name,
		Code:      project.Code,
