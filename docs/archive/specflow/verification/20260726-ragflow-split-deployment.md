...
- Scope: Task 2 的 fixture、Runbook、Python 初始化器，以及 Gateway MCP、受管 `traefik` 预检、固定 HTTP Probe、聚合 Version 和 Task 1 运行时契约的集成核对。
...
6. 聚合 Version 保留既有拆分 Version，并将 logical mount 从裸 `data` / `logs` 隔离为组件专属目录；参考 Compose 与 Orbit 的渲染布局一致。
...
| `scripts/ragflow-bundled/docker-compose.yml` | 已使用 `./data/mysql`、`./data/redis`、`./data/minio`、`./data/es01` 和 `./data/ragflow-cpu/logs` 独立 bind mount。 |
...
| 聚合 Version 的数据目录隔离与运行时验收 | 通过 | `v0.26.4-cpu-elasticsearch-bundled-isolated-data` 的五个组件均为 `healthy`，稳定性窗口无重启，`ragflow-cpu:80/` Probe 为 `reachable`。 |

## Test results

| 命令 | 结果 |
| --- | --- |
| `make -C mcp check` | 通过：Ruff、format、mypy、pytest；27 passed，1 个 Docker 集成测试按默认标记排除。 |
...
| `uv --directory mcp run python ../scripts/test_ragflow_initialize.py` | 通过：17 tests。 |
| `go test ./internal/application/deployment/usecase` | 通过。 |
| `python scripts/test_ragflow_initialize.py` | 通过：17 tests。 |
| `docker compose --env-file .env.example -f docker-compose.yml config --quiet`（`scripts/ragflow-bundled`） | 通过；聚合参考 Compose 的目录配置可静态解析。 |
| MCP `verify_deployment` 与 `runtime_http_probe` | 通过；聚合 Deployment 在 MySQL 首次初始化完成后达到 `consistent`，HTTP 入口可达。 |
...
4. 聚合 Version 最初复用了拆分 Fixture 的裸 `data` mount，导致多个组件落入同一物理 Service 目录，MySQL 无法可靠启动。该 Version 未被修改；新聚合 Version 使用组件命名空间目录并通过 MCP 运行时验收。
...
4. Orbit 当前不能编码 Compose `depends_on.condition: service_healthy`。MySQL 首次初始化期间 RAGFlow 会重试连接；验收必须等待稳定性窗口和 HTTP Probe，不能以首次日志中的连接拒绝判定最终失败。

## Conclusion

Task 2 已完成拆分与聚合两条真实闭环：Gateway 以 `http://localhost:8080` 预检 bridge `traefik` 网络；拆分 Version 的四个依赖和 RAGFlow 均经 MCP 创建、发布、部署并验证为 `consistent`。聚合 Version `v0.26.4-cpu-elasticsearch-bundled-isolated-data` 使用组件隔离目录，五个容器健康、稳定性窗口无重启，RAGFlow 的受管 HTTP Probe 可达。运行记录位于 `data/ragflow/runs/`，不包含实际 Credential value。
