...
```text
<environment.workspace_root>/<service-code>/tei/cache/bge-m3
```

`environment.workspace_root` 是所选部署 Environment 已保存的工作目录，不是 YAML 配置项。
...
```powershell
python skills/_ragflow/prepare_ragflow_tei.py stage-model --model-dir <environment.workspace_root>/<service-code>/tei/cache/bge-m3 --source-dir <environment.workspace_root>/<source-service-code>/tei/cache/bge-m3
```
...
```powershell
git lfs install
git lfs clone https://huggingface.co/BAAI/bge-m3 <environment.workspace_root>/<service-code>/tei/cache/bge-m3
git -C <environment.workspace_root>/<service-code>/tei/cache/bge-m3 lfs fsck
```
...
```powershell
hf download BAAI/bge-m3 --revision 5617a9f61b028005a4858fdac845db406aefb181 --local-dir <environment.workspace_root>/<service-code>/tei/cache/bge-m3 --dry-run
```
...
python skills/_ragflow/prepare_ragflow_tei.py check --profile cpu --model-dir <environment.workspace_root>/<service-code>/tei/cache/bge-m3
python skills/_ragflow/prepare_ragflow_tei.py check --profile gpu --model-dir <environment.workspace_root>/<service-code>/tei/cache/bge-m3

# 显式执行：按 Git LFS 或 HF CLI 准备固定 revision 的模型
python skills/_ragflow/prepare_ragflow_tei.py prepare-model --source git-lfs --model-dir <environment.workspace_root>/<service-code>/tei/cache/bge-m3
python skills/_ragflow/prepare_ragflow_tei.py prepare-model --source hf-cli --model-dir <environment.workspace_root>/<service-code>/tei/cache/bge-m3

# 显式压缩备份已校验模型，并恢复到空缓存目录供下一轮测试
python skills/_ragflow/prepare_ragflow_tei.py backup-model --archive /srv/orbit-backups/bge-m3-<revision>.tar.gz --model-dir <environment.workspace_root>/<service-code>/tei/cache/bge-m3
python skills/_ragflow/prepare_ragflow_tei.py restore-model --archive /srv/orbit-backups/bge-m3-<revision>.tar.gz --model-dir <environment.workspace_root>/<service-code>/tei/cache/bge-m3

# 显式拉取已验证的 TEI image digest，并检查备用镜像站
python skills/_ragflow/prepare_ragflow_tei.py prepare-image --profile cpu --model-dir <environment.workspace_root>/<service-code>/tei/cache/bge-m3
python skills/_ragflow/prepare_ragflow_tei.py prepare-image --profile gpu --model-dir <environment.workspace_root>/<service-code>/tei/cache/bge-m3

# 按 profile 执行所有经明确授权的准备步骤
python skills/_ragflow/prepare_ragflow_tei.py prepare --profile cpu --model-dir <environment.workspace_root>/<service-code>/tei/cache/bge-m3
```
