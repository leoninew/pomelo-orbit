...
现有 `GET /api/pipeline/:pipeline_id` 的 `variable_declarations` 会返回阶段变量的 `stage_id`、`stage_name`，但没有稳定的变量类型字段。`source` 混合了持久化来源和运行期内部语义，前端不能用它推断类型。详情查询也不会读取绑定 Repository 的当前自定义变量，Repository 变量只在创建 Run 时加入解析。若继续由前端重解析脚本、制品 Liquid 或拼接 Repository 接口，会形成不可靠的展示逻辑。
...
- 让配置页面展示配置值，让 Run 详情展示该次 Run 的已冻结实际值。
- 将 Application Pipeline 绑定 Repository 的当前自定义变量直接列入全局变量；它们仍只能在 Repository 管理。
- 使所有变量查询接口按稳定类型、作用域和阶段绑定输出公共变量读模型；各变量页面复用同一种分类展示，前端不解析 Liquid 或内部 `source` 字符串来分类。
...
- “阶段变量”按 Stage 分组或按选中的 Stage 筛选。每行显示变量类型、绑定阶段、继承的全局配置和该 Stage 覆盖。
- 类型直接使用固定英文原语，不显示取值来源。

### Variable Classification And Read Model

所有变量查询接口的变量列表应是服务端组装的读模型，而非持久化 `VariableDeclaration` 的直接映射。变量分类采用以下三项稳定的 `kind`：

| `kind` | 语义 | 默认作用域与操作 |
|---|---|---|
| `system_generated` | Run 创建时生成的只读值，如 `runtime_datetime` | `global`；不可配置 |
| `repository_context` | Repository 内置上下文和自定义变量，如 `repository_code`、`repository_url`、`repository_ref` | `global`；Repository 自定义变量仅在 Repository 中管理 |
| `pipeline_context` | Pipeline 配置和 Stage Liquid 输入，如 `image_name`、`working_dir` | `global` 或 `stage`；可按现有边界管理或覆盖 |

每个条目还必须独立表达：

- `scope`：`global` 或 `stage`，禁止由空 `stage_id` 让前端猜测。
- `stage_binding`：阶段作用域条目的 `{ stage_id, stage_name }`；全局条目为空。
- 面向配置展示的全局配置、阶段覆盖和可编辑性。敏感值继续遵守既有脱敏规则。

同名全局变量在全局 Tab 保持一个全局条目；每个 Stage 在阶段 Tab 有自己的带阶段绑定的 `pipeline_context` 条目。阶段覆盖是该 Stage 变量的配置状态，而不是靠前端将同名全局行复制或重新解析得出。
...
- `image_name`、`repository_dockerfile`、`working_dir` 等 Stage 变量按绑定 Stage 展示；前端和后端使用同名变量时可分别配置和比较。
- Repository、Pipeline、Pipeline Snapshot 和 Pipeline Run 的变量条目均显式提供三类 `kind`、`scope`、`stage_binding` 和配置片段；前端不重解析脚本、制品或内部 `source` 来补齐这些信息。
...
- `kind`、`scope` 和 `stage_binding` 是所有变量响应的公开语义契约；内部 `source` 不再承担前端分类责任。
