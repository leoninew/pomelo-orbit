...
- SSH credential 在包中使用 `private_key` 业务字段，而非数据库的 `encrypted_private_key`。导出时以源 key 解密，导入时用目标 key 加密；包以外不暴露持久化密文。其他密钥、PEM 和 token 直接使用业务值。
...
- 导入 Application 在交给仓储前将所有 Service status 规范化为 `stopped`、所有 Route `enabled` 规范化为 `false`。该规范化不改变包的其他业务字段。
...
- 增加 `POST /api/project/handover`。请求为 `multipart/form-data`，含 `file`、`mode`（`new` 或 `replace`）和仅 `replace` 使用的 `target_project_id`。Handler 读取受限文件内容并将当前认证用户、模式、目标 ID、JSON bytes 交给 Application；成功以现有 `ProjectResp` 返回目标 Project。
...
2. 新增 `internal/application/project_handover/{dto,usecase}`。DTO 定义 Handover JSON package v1；usecase 组合领域定义生成包，严格编解码后按依赖顺序调用这些领域能力。它检查 Project / member 权限，调用 `common/crypto` 做环境私钥解密 / 加密，维护源 ID 到目标 ID 的映射，规范化 Service status / Route enabled，并依赖 request UoW 保证整体回滚。
...
2. Application 集成测试验证模式 A 创建新 Project 和当前用户成员、模式 B 通过领域操作替换目标 CD 数据但保留原成员、源 / 目标相同 Project ID 无影响、源记录 ID 不写入目标、私钥跨 key 的解密 / 重加密、Service 强制 stopped、Route 强制 disabled。
