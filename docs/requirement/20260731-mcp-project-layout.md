...
原有本地 stdio MCP 使用不一致的交付控制面标识，但职责是 Application、Version、Service、Gateway 的交付生命周期及运行时验收，不是持续集成。未来还需要独立的持续集成/流水线 MCP，不能继续让两个职责共享一个含义不清的项目根目录。
...
1. 将当前交付控制面统一命名为 `pomelo-orbit-mcp`。
2. 将 MCP 工程统一组织为 `mcp/src/delivery-mcp/` 与 `mcp/src/pipeline-mcp/` 两个并列项目。
3. 交付 MCP 保留 Pomelo Orbit 控制面适配语义和 `orbit_*` 工具名，命令、配置和文档统一使用 `pomelo-orbit-mcp`。
...
3. 不保留旧注册名、Python 包或旧目录入口的兼容层。
4. 不启动开发服务器或执行部署生命周期写操作。
5. 不将集成式 RAGFlow 数据迁移、挂载或复用到拆分式拓扑，反之亦然。

## User scenarios

1. Codex 在新会话通过 `pomelo-orbit-mcp` 发现交付控制工具，并从 `mcp/src/delivery-mcp` 启动本地 stdio Server。
...
2. Delivery 项目提供 `pomelo-orbit-mcp` 命令，并以 `pomelo_orbit_mcp` 包实现；`.codex/config.toml` 注册 `pomelo-orbit-mcp`。
...
5. AGENTS、MCP 指南、RAGFlow 运维手册和技能元数据明确新的命令与注册名；工具仍以 `mcp__pomelo-orbit-mcp__orbit_*` 出现。
...
8. 两个 RAGFlow skill 都要求初始化上述完整库存、在每次 lifecycle write 前读取 `pomelo-orbit-mcp` schema，并只启动自己拥有的 RAGFlow 服务。
...
1. `pomelo_orbit_mcp` 是交付 MCP 的 Python 包名；`orbit_*` 保留为外部控制面适配工具名。
