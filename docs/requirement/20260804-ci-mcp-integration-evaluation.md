...
本评估范围包括现有 CI 领域和 HTTP API、当前会话实际可用的 MCP 工具与资源，以及独立 CI MCP 和既有 `pomelo-orbit-mcp` 的职责边界。结论与后续计划仅描述建议，不进行 MCP、HTTP API、数据库、流水线或部署的写入。
...
4. 本次会话发现的 `pomelo-orbit-mcp` MCP 暴露 `orbit_*` 控制面工具和 `runtime_*`、`verify_deployment` 运行态只读工具。MCP resources 仅包含终端服务信息与文件传输状态，不包含 CI 的项目、模板、运行或日志资源。

## MCP 能力与集成边界

| 边界 | 归属 | 结论 |
| --- | --- | --- |
| Application、Version、Service、Gateway 生命周期 | `pomelo-orbit-mcp` 的 `orbit_*` | 保持现状；其写操作只完成用户明确要求的单一交付动作。 |
| 受管 Docker Compose 状态、日志、网络、探针与部署验证 | `pomelo-orbit-mcp` 的 `runtime_*` 与 `verify_deployment` | 保持受管 target 和只读诊断限制；不能作为 CI 运行的替代日志或执行接口。 |
...
`pomelo-orbit-mcp` 的 Server instructions 明确其面向 Orbit 生命周期写入，运行态工具只读且目标必须受 Orbit 管理。因此，把 CI 工具加入该 Server 会混淆权限、资源标识和审计语义；当前 `mcp/src/pipeline-mcp/` 的独立预留方向与此结论一致。
...
2. 不改动 `pomelo-orbit-mcp` 的工具表、Orbit HTTP API 或 Docker Compose 生命周期边界。
...
写工具的结果必须返回稳定的资源标识、当前状态和简短动作摘要，默认不返回变量值、仓库凭据、完整脚本、运行环境或原始日志。日志读取应复用现有 `offset` 与 `is_complete` 契约，并限制单次结果大小。跨 MCP 的 CI->CD 工作流必须由调用方显式串联：先读取并确认 CI 成功，再由用户单独授权 `pomelo-orbit-mcp` 部署。

## 验收标准

1. 方案明确区分 CI MCP、`pomelo-orbit-mcp` 和 GitHub Actions 的职责，并禁止 CI 成功后自动部署。
...
2. 2026-08-04：选择独立 `pipeline-mcp`，不扩展 `pomelo-orbit-mcp`。
