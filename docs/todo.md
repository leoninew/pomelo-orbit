...
- ✅ **修复 token 过期后受保护页面放行问题**：已修复路由守卫逻辑，`fetchUser()` 返回 `undefined` 时正确跳转登录页
- ✅ **保护系统角色**：已添加保护逻辑，防止删除或修改 admin 角色的 code
- **优化角色列表权限查询**：角色列表响应当前逐个查询 `permission_codes`，后续改为批量查询组装，避免 N+1 查询（低优先级）
