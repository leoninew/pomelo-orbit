	...
	<div class="flex flex-col gap-4">
		<div class="flex items-center justify-between flex-wrap gap-2">
			...
			<label class="input input-sm input-bordered flex items-center gap-2">
				...
				<input v-model="searchText" type="text" placeholder="搜索用户名" class="w-36" @keyup.enter="handleSearch" />
			</label>
		</div>
		...
		<div class="card bg-base-100 shadow-sm overflow-x-auto">
			<table class="table">
				...
				<tbody>
					...
					<tr v-else-if="history.length === 0"><td colspan="5" class="text-center py-8 text-base-content/60">暂无记录</td></tr>
...
async function fetchHistory() {
	try {
		await execute(async () => {
			const res = await authApi.listLoginHistory({ page: pagination.current, per_page: pagination.pageSize, search: searchText.value || undefined });
			history.value = res.items; pagination.total = res.total;
		});
	} catch { toast.error('获取登录历史失败'); }
}

function handleSearch() { pagination.current = 1; fetchHistory(); }
function goPage(p: number) { pagination.current = p; fetchHistory(); }
