		...
		<div class="card bg-base-100 shadow-sm">
			<div class="card-body p-5">
				<div class="flex items-center justify-between mb-3">
					...
					<button class="btn btn-sm btn-primary" @click="passwordModalRef?.showModal()">
						修改密码
					</button>
				</div>
				<dl v-if="authStore.user" class="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-3 text-sm">
					<div class="flex gap-2">
						<dt class="text-base-content/70 w-24 shrink-0">用户名</dt>
						<dd class="font-medium">{{ authStore.user.username }}</dd>
					</div>
					<div class="flex gap-2">
						<dt class="text-base-content/70 w-24 shrink-0">创建时间</dt>
						<dd>{{ formatTime(authStore.user.created_at) }}</dd>
					</div>
					<div class="flex gap-2">
						<dt class="text-base-content/70 w-24 shrink-0">上次登录</dt>
						<dd>{{ formatTime(authStore.user.last_login_at) }}</dd>
					</div>
				</dl>
			</div>
		</div>
								...
								<td class="text-base-content/60">
									<span v-if="typeof item.default === 'boolean'">
										<span
											...
											:class="item.default ? 'badge-outline badge-success' : 'badge-ghost'"
										>
											{{ item.default ? '已启用' : '未启用' }}
										</span>
									</span>
									<span v-else>
								...
								<td>
									<template v-if="editingKey === item.key">
										<input
											...
											:checked="editingBool"
											@change="editingBool = ($event.target as HTMLInputElement).checked"
										/>
										<select
										...
										>
											<option v-for="opt in selectOptions[item.key]" :key="opt" :value="opt">
												{{ opt }}
											</option>
										</select>
										<input
											v-else-if="secretKeys.has(item.key)"
											v-model="editingStr"
											type="password"
		...
		<dialog ref="resetModalRef" class="modal">
			<div class="modal-box">
				...
				<p class="py-4 text-sm">
					确定将
					<code class="text-xs">{{ pendingResetKey }}</code>
					重置为默认值？
				</p>
			...
			<div class="modal-box w-full max-w-md">
				...
				<div class="flex flex-col gap-3">
					<fieldset class="fieldset">
						...
						/>
						<p v-if="passwordErrors.old_password" class="fieldset-label text-error">
					...
					<fieldset class="fieldset">
						...
						<input
							...
							placeholder="至少 6 位"
						/>
						<p v-if="passwordErrors.new_password" class="fieldset-label text-error">
					...
					<fieldset class="fieldset">
						...
						/>
						<p v-if="passwordErrors.confirm_password" class="fieldset-label text-error">
