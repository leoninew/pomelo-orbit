			...
			<div class="card-body p-5">
				<div class="flex items-center justify-between mb-4">
					...
					<div v-if="routeData" class="flex items-center gap-2">
						...
						<button v-if="routeData.enabled" class="btn btn-sm btn-warning btn-ghost" :disabled="operating" @click="handleDisable">停用</button>
						<button v-else class="btn btn-sm btn-success btn-ghost" :disabled="operating" @click="handleEnable">启用</button>
						<button class="btn btn-sm btn-error btn-ghost" :disabled="routeData.enabled" @click="deleteModalRef?.showModal()">删除</button>
					</div>
				</div>
					...
					<div v-if="!routeData.https_enabled" class="flex flex-col gap-3">
						...
						<div class="flex items-center gap-2 flex-wrap">
							<label class="btn btn-sm btn-ghost gap-1.5 cursor-pointer">
								<Upload class="size-4" />上传证书 (PEM)
								<input type="file" accept=".pem,.crt,.cer" class="hidden" @change="handleCertUpload" />
							</label>
							<button class="btn btn-sm btn-primary" :disabled="!canUseLetsencrypt || operating" :title="!canUseLetsencrypt ? `Let's Encrypt 不支持内网域名或IP` : ''" @click="handleEnableLetsencrypt">
								使用 Let's Encrypt
							</button>
							<button class="btn btn-sm btn-ghost" :disabled="operating" @click="handleEnableMkcert">使用 mkcert</button>
						</div>
					</div>
					<div v-else class="flex flex-col gap-3">
						<div role="alert" class="alert alert-success text-sm">
							{{ routeData.cert_type === 'letsencrypt' ? "使用 Let's Encrypt 自动证书" : routeData.cert_type === 'mkcert' ? '使用 mkcert 本地证书' : 'HTTPS 已启用（手动证书）' }}
						</div>
						<button class="btn btn-sm btn-error btn-ghost w-fit" :disabled="operating" @click="handleDisableHttps">
			...
			<div class="modal-box w-full max-w-lg">
				...
				<div class="flex flex-col gap-3">
					<label class="form-control w-full">
						...
						<input :value="form.name" type="text" class="input input-bordered input-sm opacity-60" disabled />
					</label>
					<label class="form-control w-full">
						<div class="label pb-1"><span class="label-text">域名</span></div>
						<input v-model="form.domain" type="text" class="input input-bordered input-sm" :class="{ 'input-error': errors.domain }" />
					...
					<label class="form-control w-full">
						...
						<input v-model="form.target_url" type="text" class="input input-bordered input-sm" :class="{ 'input-error': errors.target_url }" />
...
const canUseLetsencrypt = computed(() => {
	...
	return d !== 'localhost' && !d.endsWith('.localhost') && !d.endsWith('.lvh.me') && !/^\d+\.\d+\.\d+\.\d+$/.test(d);
});
...
async function fetchRoute() {
	try {
		await execute(async () => {
			...
			Object.assign(form, { name: data.name, domain: data.domain, path_prefix: data.path_prefix, target_url: data.target_url, enabled: data.enabled });
		});
	} catch { toast.error('获取路由详情失败'); router.push('/cd/routes'); }
}

function openEditModal() { Object.assign(errors, { domain: '', target_url: '' }); editModalRef.value?.showModal(); }
...
async function handleSave() {
	...
	try {
		await executeOp(async () => {
			const updateData = { domain: form.domain, path_prefix: form.path_prefix, target_url: form.target_url, enabled: form.enabled };
			await routeApi.update(routeId, updateData);
			toast.success('更新成功'); editModalRef.value?.close(); fetchRoute();
		});
	} catch (error) { toast.error(error instanceof Error ? error.message : '更新失败'); }
}

async function handleEnable() {
	try { await executeOp(async () => { await routeApi.enable(routeId); toast.success('路由已启用'); fetchRoute(); }); }
	catch (error) { toast.error(error instanceof Error ? error.message : '启用失败'); }
}

async function handleDisable() {
	try { await executeOp(async () => { await routeApi.disable(routeId); toast.success('路由已停用'); fetchRoute(); }); }
	catch (error) { toast.error(error instanceof Error ? error.message : '停用失败'); }
}

async function handleDelete() {
	try { await executeOp(async () => { await routeApi.delete(routeId); toast.success('删除成功'); router.push('/cd/routes'); }); }
...
async function handleCertUpload(event: Event) {
	...
	if (!file) return;
	try { await executeOp(async () => { await routeApi.uploadCert(routeId, file); toast.success('证书上传成功，HTTPS 已启用'); fetchRoute(); }); }
	catch (error) { toast.error(error instanceof Error ? error.message : '证书上传失败'); }
}

async function handleDisableHttps() {
	try { await executeOp(async () => { await routeApi.disableHttps(routeId); toast.success('HTTPS 已禁用'); fetchRoute(); }); }
	catch (error) { toast.error(error instanceof Error ? error.message : '操作失败'); }
}

async function handleEnableLetsencrypt() {
	try { await executeOp(async () => { await routeApi.enableLetsencrypt(routeId); toast.success("Let's Encrypt 证书已启用"); fetchRoute(); }); }
	catch (error) { toast.error(error instanceof Error ? error.message : '操作失败'); }
}

async function handleEnableMkcert() {
	try { await executeOp(async () => { await routeApi.enableMkcert(routeId); toast.success('mkcert 证书已生成并启用'); fetchRoute(); }); }
	catch (error) { toast.error(error instanceof Error ? error.message : '操作失败'); }
}
