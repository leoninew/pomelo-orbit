...
func containerLogsSinceCommand(since string) composeCommand {
	return composeCommand{Name: "docker", Args: []string{"compose", "-f", "docker-compose.yml", "logs", "--since", since}}
}

func containerLogsTailCommand(tail string) composeCommand {
	return composeCommand{Name: "docker", Args: []string{"compose", "-f", "docker-compose.yml", "logs", "--tail", tail}}
}
