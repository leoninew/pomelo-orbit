...
func (s Service) ExportApplication(ctx context.Context, userId string, applicationId string) (cdto.ApplicationExport, error) {
	...
	if err != nil {
		return cdto.ApplicationExport{}, apperror.Wrap(apperror.KindInternal, "Failed to load application config files", err)
	}
	serviceConfigs, err := s.store.ServiceConfigs(ctx, app.Id)
	if err != nil {
		return cdto.ApplicationExport{}, apperror.Wrap(apperror.KindInternal, "Failed to load application service configs", err)
	}
	routes, err := s.store.Routes(ctx, app.Id)
	if err != nil {
		return cdto.ApplicationExport{}, apperror.Wrap(apperror.KindInternal, "Failed to load application routes", err)
	}
	return cdto.ApplicationExport{Application: app, ConfigFiles: files, ServiceConfigs: serviceConfigs, Routes: routes}, nil
}
...
func (s Service) CreateApplicationFile(ctx context.Context, userId string, applicationId string, input cdto.ConfigFileInput) (model.ApplicationConfigFile, error) {
	app, err := s.loadApplicationForUser(ctx, userId, applicationId)
	if err != nil {
...
func (s Service) UpdateApplicationFile(ctx context.Context, userId string, applicationId string, fileId string, input cdto.ConfigFileInput) (model.ApplicationConfigFile, error) {
...
func (s Service) CreateApplicationRoute(ctx context.Context, userId string, applicationId string, input cdto.ApplicationRouteInput) (model.ApplicationRoute, error) {
	app, err := s.loadApplicationForUser(ctx, userId, applicationId)
	if err != nil {
...
func (s Service) UpdateApplicationRoute(ctx context.Context, userId string, applicationId string, routeId string, input cdto.ApplicationRouteInput) (model.ApplicationRoute, error) {
...
func (s Service) ApplicationComposeServices(ctx context.Context, userId string, applicationId string) ([]cdto.ApplicationServiceConfigView, error) {
	views, err := s.applicationServiceConfigViews(ctx, userId, applicationId)
	if err != nil {
		return nil, err
	}
	resp := make([]cdto.ApplicationServiceConfigView, 0, len(views))
	for _, view := range views {
		resp = append(resp, cdto.ApplicationServiceConfigView{ServiceName: view.ServiceName, DefaultDomain: view.DefaultDomain, DefaultPort: view.DefaultPort})
	}
	return resp, nil
}

func (s Service) ApplicationServiceConfigs(ctx context.Context, userId string, applicationId string) ([]cdto.ApplicationServiceConfigView, error) {
	return s.applicationServiceConfigViews(ctx, userId, applicationId)
}

func (s Service) UpdateApplicationServiceConfig(ctx context.Context, userId string, applicationId string, serviceName string, imageInput *string) (cdto.ApplicationServiceConfigView, error) {
	app, compose, err := s.loadApplicationCompose(ctx, userId, applicationId)
	if err != nil {
		return cdto.ApplicationServiceConfigView{}, err
	}
	...
	if !exists {
		return cdto.ApplicationServiceConfigView{}, apperror.New(apperror.KindNotFound, "Service "+serviceName+" not found")
	}
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			...
			return cdto.ApplicationServiceConfigView{}, apperror.Wrap(apperror.KindInternal, "Failed to load application service config", err)
		}
	}
	config.Image = image
	if err := s.store.UpsertApplicationServiceConfig(ctx, config); err != nil {
		return cdto.ApplicationServiceConfigView{}, apperror.Wrap(apperror.KindInternal, "Failed to update application service config", err)
	}
...
func (s Service) applicationServiceConfigViews(ctx context.Context, userId string, applicationId string) ([]cdto.ApplicationServiceConfigView, error) {
	app, compose, err := s.loadApplicationCompose(ctx, userId, applicationId)
	if err != nil {
		return nil, err
	}
	...
	resp := make([]cdto.ApplicationServiceConfigView, 0, len(services))
...
func (s Service) applicationServiceConfigView(app model.Application, serviceName string, raw any, config *model.ApplicationServiceConfig) cdto.ApplicationServiceConfigView {
	...
	return cdto.ApplicationServiceConfigView{ServiceName: serviceName, DefaultDomain: app.Code + "." + s.cfg.Traefik.DomainSuffix, DefaultPort: extractDefaultPort(service), BaseImage: baseImage, Image: image, ConfigId: configId, CreatedAt: createdAt, UpdatedAt: updatedAt}
}
