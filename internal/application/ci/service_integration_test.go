...
func TestCredentialServiceEncryptsExportsAndRejectsDuplicates(t *testing.T) {
	...
	created, err := service.CreateCredential(ctx, ciTestUserId, CredentialCreateInput{ProjectId: ciTestProjectId, Name: "GitHub Token", Type: "github_token", Data: plainData})
	if err != nil {
		t.Fatal(err)
	}
	...
	if exported.Version != CredentialExportVersion || exported.Name != "GitHub Token" || exported.Type != "github_token" || exported.Data != plainData {
		t.Fatalf("unexpected exported credential: %+v", exported)
	}

	if _, err := service.CreateCredential(ctx, ciTestUserId, CredentialCreateInput{ProjectId: ciTestProjectId, Name: "GitHub Token", Type: "github_token", Data: "other"}); err == nil || apperror.StatusCode(err) != http.StatusConflict {
		t.Fatalf("expected duplicate credential create conflict, got %v", err)
	}

	other, err := service.CreateCredential(ctx, ciTestUserId, CredentialCreateInput{ProjectId: ciTestProjectId, Name: "Other Token", Type: "github_token", Data: "other"})
	if err != nil {
		t.Fatal(err)
	}
	if _, err := service.UpdateCredential(ctx, ciTestUserId, other.Id, CredentialUpdateInput{Name: stringPtr("GitHub Token")}); err == nil || apperror.StatusCode(err) != http.StatusConflict {
...
func TestRepositoryTriggerCreatesAndReusesCurrentTemplateSnapshot(t *testing.T) {
	...
	first, err := service.TriggerRepository(ctx, ciTestUserId, PipelineRunTriggerInput{RepositoryId: ciTestRepositoryId, TemplateId: ciTestTemplateId, TriggerRef: "main", Variables: map[string]string{"working_dir": "."}})
	if err != nil {
		t.Fatal(err)
	}
	...
	second, err := service.TriggerRepository(ctx, ciTestUserId, PipelineRunTriggerInput{RepositoryId: ciTestRepositoryId, TemplateId: ciTestTemplateId, TriggerRef: "main", Variables: map[string]string{"working_dir": "."}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestPipelineRunServiceReadsLogsAndFiltersArtifacts(t *testing.T) {
	...
	filtered, err := service.ListArtifacts(ctx, ciTestUserId, ArtifactListInput{ProjectId: ciTestProjectId, RepositoryId: ciTestRepositoryId, TemplateId: ciTestTemplateId, Search: "test.log", Page: 1, PerPage: 20})
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := service.ListArtifacts(ctx, ciTestUserId, ArtifactListInput{ProjectId: ciTestProjectId, RepositoryId: "missing-repository"}); err == nil || apperror.StatusCode(err) != http.StatusNotFound {
		t.Fatalf("expected missing repository filter to return 404, got %v", err)
	}
	if _, err := service.ListArtifacts(ctx, ciTestUserId, ArtifactListInput{ProjectId: ciTestProjectId, TemplateId: "missing-template"}); err == nil || apperror.StatusCode(err) != http.StatusNotFound {
