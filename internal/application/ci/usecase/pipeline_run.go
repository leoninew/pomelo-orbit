...
func (s Service) ListRepositoryRuns(ctx context.Context, userId string, repositoryId string, page int, perPage int) (repository.Page[cidto.PipelineRunDetail], error) {
	repo, err := s.loadRepositoryForUser(ctx, userId, repositoryId)
	if err != nil {
		return repository.Page[cidto.PipelineRunDetail]{}, err
	}
	items, err := s.store.ListPipelineRunsByRepository(ctx, repo.Id, page, perPage)
	if err != nil {
		return repository.Page[cidto.PipelineRunDetail]{}, apperror.Wrap(apperror.KindInternal, "Failed to list repository runs", err)
	}
	return s.pipelineRunDetails(ctx, items, false)
}

func (s Service) TriggerRepository(ctx context.Context, userId string, input cidto.PipelineRunTriggerInput) (cidto.PipelineRunDetail, error) {
	repo, err := s.loadRepositoryForUser(ctx, userId, input.RepositoryId)
	if err != nil {
		return cidto.PipelineRunDetail{}, err
	}
	templateId := strings.TrimSpace(input.TemplateId)
	if templateId == "" {
		return cidto.PipelineRunDetail{}, apperror.New(apperror.KindValidation, "template_id is required")
	}
	...
	if err != nil {
		return cidto.PipelineRunDetail{}, apperror.Wrap(apperror.KindInternal, "Failed to load pipeline run", err)
	}
	return s.pipelineRunDetail(ctx, created, false)
}

func (s Service) ListPipelineRuns(ctx context.Context, userId string, input cidto.PipelineRunListInput) (repository.Page[cidto.PipelineRunDetail], error) {
	projectId := strings.TrimSpace(input.ProjectId)
	if projectId == "" {
		return repository.Page[cidto.PipelineRunDetail]{}, apperror.New(apperror.KindValidation, "project_id is required")
	}
	...
	if err != nil {
		return repository.Page[cidto.PipelineRunDetail]{}, apperror.Wrap(apperror.KindInternal, "Failed to list pipeline runs", err)
	}
	return s.pipelineRunDetails(ctx, items, false)
}

func (s Service) PipelineRunForUser(ctx context.Context, userId string, runId string) (cidto.PipelineRunDetail, error) {
...
func (s Service) PipelineStageLog(ctx context.Context, userId string, runId string, stageRunId string, offset int) (cidto.PipelineStageLog, error) {
	run, err := s.loadPipelineRunForUser(ctx, userId, runId)
	if err != nil {
		return cidto.PipelineStageLog{}, err
	}
	if offset < 0 {
		return cidto.PipelineStageLog{}, apperror.New(apperror.KindValidation, "offset must be greater than or equal to 0")
	}
	...
	if err != nil {
		return cidto.PipelineStageLog{}, apperror.Wrap(apperror.KindInternal, "Failed to read stage log", err)
	}
	return cidto.PipelineStageLog{Logs: string(content), Offset: newOffset, IsComplete: pipelineRunStatusComplete(stageRun.Status)}, nil
}

func (s Service) CancelPipelineRun(ctx context.Context, userId string, runId string) (cidto.PipelineRunDetail, error) {
	run, err := s.loadPipelineRunForUser(ctx, userId, runId)
	if err != nil {
		return cidto.PipelineRunDetail{}, err
	}
	if run.Status != status.WorkStatusWaitingToRun && run.Status != status.WorkStatusRunning {
		return cidto.PipelineRunDetail{}, apperror.New(apperror.KindValidation, "Cannot cancel run with status "+run.Status)
	}
	if err := s.store.CancelPipelineRun(ctx, run.Id); err != nil {
		return cidto.PipelineRunDetail{}, apperror.Wrap(apperror.KindInternal, "Failed to cancel pipeline run", err)
	}
...
func (s Service) RetryPipelineRun(ctx context.Context, userId string, runId string) (cidto.PipelineRunDetail, error) {
	...
	if original.Status != status.WorkStatusFaulted && original.Status != status.WorkStatusRanToCompletion {
		return cidto.PipelineRunDetail{}, apperror.New(apperror.KindValidation, "Cannot retry run with status "+original.Status)
	}
...
func (s Service) pipelineRunDetails(ctx context.Context, page repository.Page[model.PipelineRun], includeStages bool) (repository.Page[cidto.PipelineRunDetail], error) {
	items := make([]cidto.PipelineRunDetail, 0, len(page.Items))
	...
	return repository.Page[cidto.PipelineRunDetail]{Items: items, Total: page.Total, Page: page.Page, PerPage: page.PerPage}, nil
}

func (s Service) pipelineRunDetail(ctx context.Context, item model.PipelineRun, includeStages bool) (cidto.PipelineRunDetail, error) {
	...
	if includeStages {
		...
		if err != nil {
			return cidto.PipelineRunDetail{}, apperror.Wrap(apperror.KindInternal, "Failed to list stage runs", err)
		}
		stageRuns = items
	}
	return cidto.PipelineRunDetail{Run: item, VariablesSnapshot: variables, StageRuns: stageRuns}, nil
}
