...
func New(store RepositoryStore, tasks ciport.TaskService, dataRoot string, secretKey string, logger *slog.Logger, logStore ciport.LogReader) Service {
	return NewWithRunner(store, tasks, dataRoot, secretKey, logger, cirunner.DockerRunner{}, logStore)
}

func NewWithRunner(store RepositoryStore, tasks ciport.TaskService, dataRoot string, secretKey string, logger *slog.Logger, runner ciport.ContainerRunner, logStore ciport.LogReader) Service {
...
func NewExecutionService(store PipelineExecutionStore, dataRoot string, secretKey string, logger *slog.Logger, runner ciport.ContainerRunner, logStore ciport.LogReader) Service {
	workspace := ciworkspace.New(dataRoot)
