...
func (s Service) PipelineSnapshotForUser(ctx context.Context, userId string, snapshotId string) (cidto.PipelineSnapshotDetail, error) {
	...
	if snapshot.ProjectId != nil {
		if err := s.ensureProjectMembership(ctx, *snapshot.ProjectId, userId); err != nil {
			return cidto.PipelineSnapshotDetail{}, err
		}
	}
	...
	return cidto.PipelineSnapshotDetail{Snapshot: snapshot, StagesSnapshot: stages, VariablesSnapshot: variables}, nil
}
...
func (s Service) createPipelineSnapshot(ctx context.Context, template model.PipelineTemplate) (model.PipelineSnapshot, error) {
	...
	snapshot := model.PipelineSnapshot{Id: idutil.NewId(), ProjectId: template.ProjectId, TemplateId: template.Id, Version: template.Version, StagesSnapshot: string(stageData), VariablesSnapshot: string(variableData)}
