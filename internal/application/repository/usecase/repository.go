...
func (s Service) CreateRepository(ctx context.Context, userId string, input repositorydto.RepositoryCreateInput) (repositorydto.RepositoryDetail, error) {
	...
	if err != nil {
		return repositorydto.RepositoryDetail{}, apperror.Wrap(apperror.KindInternal, "Failed to load repository", err)
	}
	return s.repositoryDetail(ctx, created)
}

func (s Service) RepositoryForUser(ctx context.Context, userId string, repositoryId string) (repositorydto.RepositoryDetail, error) {
...
func (s Service) UpdateRepository(ctx context.Context, userId string, repositoryId string, input repositorydto.RepositoryUpdateInput) (repositorydto.RepositoryDetail, error) {
	repo, err := s.loadRepositoryForUser(ctx, userId, repositoryId)
	if err != nil {
		return repositorydto.RepositoryDetail{}, err
	}
...
func (s Service) CreateRepositoryWebhook(ctx context.Context, userId string, repositoryId string, input repositorydto.WebhookCreateInput) (model.RepositoryWebhook, error) {
	repo, err := s.loadRepositoryForUser(ctx, userId, repositoryId)
	if err != nil {
...
func (s Service) UpdateRepositoryWebhook(ctx context.Context, userId string, repositoryId string, webhookId string, input repositorydto.WebhookUpdateInput) (model.RepositoryWebhook, error) {
	repo, err := s.loadRepositoryForUser(ctx, userId, repositoryId)
	if err != nil {
		return model.RepositoryWebhook{}, err
	}
...
func (s Service) ReceiveRepositoryWebhook(ctx context.Context, input repositorydto.WebhookReceiveInput) (repositorydto.WebhookReceiveResult, error) {
	webhook, err := s.loadRepositoryWebhook(ctx, input.WebhookId)
	if err != nil {
		return repositorydto.WebhookReceiveResult{}, err
	}
	if !webhook.Enabled {
		return repositorydto.WebhookReceiveResult{Status: "ignored", Reason: "webhook disabled"}, nil
	}
	var body map[string]any
	if err := json.Unmarshal(input.Payload, &body); err != nil {
		return repositorydto.WebhookReceiveResult{}, apperror.New(apperror.KindValidation, "Invalid JSON payload")
	}
	...
	if webhook.BranchFilter == nil || (*webhook.BranchFilter != "*" && !matchBranchFilter(branch, *webhook.BranchFilter)) {
		return repositorydto.WebhookReceiveResult{Status: "ignored", Reason: "branch filtered"}, nil
	}
	repo, err := s.store.Repository(ctx, webhook.RepositoryId)
	if err != nil {
		if errors.Is(err, repository.ErrNotFound) {
			return repositorydto.WebhookReceiveResult{}, apperror.New(apperror.KindNotFound, "Repository "+webhook.RepositoryId+" not found")
		}
	...
	if err != nil {
		if errors.Is(err, repository.ErrNotFound) {
			return repositorydto.WebhookReceiveResult{}, apperror.New(apperror.KindNotFound, "Pipeline template "+webhook.TemplateId+" not found")
		}
		return repositorydto.WebhookReceiveResult{}, apperror.Wrap(apperror.KindInternal, "Failed to load pipeline template", err)
	}
...
func (s Service) repositoryDetail(ctx context.Context, repo model.Repository) (repositorydto.RepositoryDetail, error) {
	...
	return repositorydto.RepositoryDetail{Repository: repo, GitCredentialName: credentialName, VariableDeclarations: variables}, nil
}
...
func repositoryBuiltinVariableDeclaration(name string, defaultValue any) map[string]any {
	return map[string]any{"name": name, "description": pipelinevariable.PipelineTemplateBuiltinVariableSpecs()[name], "default": defaultValue, "value": nil, "secret": false, "source": "repository", "editable": name == "repository_ref"}
}
...
func sanitizeRepositoryVariables(variables []map[string]any) []map[string]any {
	...
	for _, variable := range variables {
		...
		if name == "" || pipelinevariable.IsPipelineTemplateBuiltinVariable(name) {
			continue
		}
		copy := map[string]any{}
...
func normalizeWebhookCreateInput(input repositorydto.WebhookCreateInput) (string, string, string, *string, error) {
	name := strings.TrimSpace(input.Name)
	templateId := strings.TrimSpace(input.TemplateId)
	secret := strings.TrimSpace(input.Secret)
