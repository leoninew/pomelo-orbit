...
func TestRoleServiceCreateUpdateAndDelete(t *testing.T) {
	...
	created, err := service.Create(ctx, SaveInput{Code: "auditor", Name: "Auditor", Description: &description, PermissionCodes: []string{"user:read"}})
	if err != nil {
		t.Fatal(err)
	}
	...
	updated, err := service.Update(ctx, SaveInput{Role: created, Code: "auditor", Name: "Auditor Updated", PermissionCodes: []string{"role:read", "user:read"}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestRoleServiceRejectsMissingPermissionAndDuplicatePermissions(t *testing.T) {
	...
	defer func() { _ = database.Close() }()
	ctx := context.Background()
	if _, err := service.Create(ctx, SaveInput{Code: "bad", Name: "Bad", PermissionCodes: []string{"missing:permission"}}); err == nil || apperror.StatusCode(err) != 404 {
		t.Fatalf("expected missing permission to return 404, got %v", err)
	}
	if _, err := service.Create(ctx, SaveInput{Code: "bad", Name: "Bad", PermissionCodes: []string{"user:read", "user:read"}}); err == nil || apperror.StatusCode(err) != 400 {
