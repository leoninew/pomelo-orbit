...
func TestUserServiceCreateUpdateStatusAndDelete(t *testing.T) {
	...
	created, err := service.Create(ctx, CreateInput{Username: " operator ", Password: "secret1", Email: &email})
	if err != nil {
		t.Fatal(err)
	}
	...
	updated, err := service.Update(ctx, UpdateInput{User: created, Username: stringPtr("operator2"), Password: stringPtr("newpass1"), Status: stringPtr("disabled")})
	if err != nil {
		t.Fatal(err)
	}
...
func TestUserServiceRejectsDuplicateUsernameAndEmail(t *testing.T) {
	...
	ctx := context.Background()
	email := "operator@example.test"
	if _, err := service.Create(ctx, CreateInput{Username: "operator", Password: "secret1", Email: &email}); err != nil {
		t.Fatal(err)
	}
	if _, err := service.Create(ctx, CreateInput{Username: "operator", Password: "secret1"}); err == nil || apperror.StatusCode(err) != 409 {
		t.Fatalf("expected duplicate username conflict, got %v", err)
	}
	if _, err := service.Create(ctx, CreateInput{Username: "operator2", Password: "secret1", Email: &email}); err == nil || apperror.StatusCode(err) != 409 {
