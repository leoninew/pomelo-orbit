...
func TestFactoryCreatesInMemoryActorBoundSession(t *testing.T) {
	client, err := NewFactory(func(actorUserId, projectId string) (*mcp.Server, error) {
		...
		mcp.AddTool(server, &mcp.Tool{Name: "whoami"}, func(_ context.Context, _ *mcp.CallToolRequest, _ struct{}) (*mcp.CallToolResult, map[string]any, error) {
			return nil, map[string]any{"actor_user_id": actorUserId, "project_id": projectId}, nil
		})
		return server, nil
	}).Connect(context.Background(), "actor-1", "project-1")
	if err != nil {
	...
	if string(result) != `{"actor_user_id":"actor-1","project_id":"project-1"}` {
