...
func pipelineRunModel(item pipelinerunsqlc.PipelineRun) model.PipelineRun {
	return model.PipelineRun{Id: item.Id, ProjectId: dbmodel.StringPtr(item.ProjectId), RepositoryId: item.RepositoryId, RepositoryName: item.RepositoryName, SnapshotId: item.SnapshotId, PipelineId: item.PipelineId, PipelineName: item.PipelineName, PipelineVersion: int(item.PipelineVersion), Trigger: item.Trigger, RepositoryRef: item.RepositoryRef, VariablesSnapshot: item.VariablesSnapshot, Status: item.Status, RetryOf: dbmodel.StringPtr(item.RetryOf), EnvironmentId: dbmodel.StringPtr(item.EnvironmentId), EnvironmentTargetType: dbmodel.StringPtr(item.EnvironmentTargetType), EnvironmentTargetRevision: dbmodel.Int64Ptr(item.EnvironmentTargetRevision), SSHCredentialId: dbmodel.StringPtr(item.SSHCredentialId), SSHCredentialRevision: dbmodel.Int64Ptr(item.SshCredentialRevision), StartedAt: dbmodel.TimePtr(item.StartedAt), FinishedAt: dbmodel.TimePtr(item.FinishedAt), ErrorMessage: dbmodel.StringPtr(item.ErrorMessage), CreatedAt: item.CreatedAt}
}
