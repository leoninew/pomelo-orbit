...
func (r Repository) Route(ctx context.Context, projectId, id string) (model.Route, error) {
	row, err := r.q(ctx).RouteById(ctx, routesqlc.RouteByIdParams{
		Id:        id,
...
func (r Repository) UpdateRoute(ctx context.Context, projectId string, route model.Route) error {
...
func (r Repository) DeleteRoute(ctx context.Context, projectId, id string) error {
	if err := r.q(ctx).DeleteRoute(ctx, routesqlc.DeleteRouteParams{Id: id, ProjectId: sql.NullString{String: strings.TrimSpace(projectId), Valid: true}}); err != nil {
