...
async def _target(
    ...
    return await resolve_runtime_target(client, settings, application_id, environment_id, instance_key)
...
def register_runtime_tools(mcp: FastMCP, client: OrbitClient, runtime: DockerRuntime, settings: Settings) -> None:
    ...
    async def runtime_doctor(
        ...
        supplied = [application_id, environment_id, instance_key]
        if any(value is not None for value in supplied) and not all(supplied):
            raise ValueError("application_id, environment_id, and instance_key must be supplied together")
        target = None
        if application_id and environment_id and instance_key:
            target = await _target(client, settings, application_id, environment_id, instance_key)
    ...
    async def runtime_compose_config(
        application_id: str, environment_id: str, instance_key: str = "default"
    ) -> dict[str, Any]:
