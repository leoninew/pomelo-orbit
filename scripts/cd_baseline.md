...
# 指定输出目录；文件名规则与未指定 --output 相同
python scripts/cd_baseline.py export --output data/exports
...
`export` 通过 SQLite `mode=ro` 打开数据库，先执行完整性和外键检查。未指定 `--output` 时，输出文件名为 `data/exports/pomelo-orbit-cd-YYYYMMDDHHMMSS.sql`。`--output` 指向已存在目录时，在该目录下使用相同的 `pomelo-orbit-cd-YYYYMMDDHHMMSS.sql` 文件名；指向文件路径时仍按该路径写入。输出使用普通 `INSERT`，不会静默跳过重复记录。`--relace`（参数名按脚本接口拼写）会在所有 `INSERT` 前按依赖反序写入该导出范围内每张表的 `DELETE`，用于恢复同一份 CD 基线。输出文件仅在导出检查通过后原子替换。
