#!/usr/bin/env python3
"""
sync_ops.py - 与上游仓库同步文件（不保留 commit 历史）

用法:
  python scripts/sync_ops.py fetch   # 从上游拉取变更到当前目录
  python scripts/sync_ops.py send    # 将当前目录 backend/frontend 变更发送到上游
...
def rsync(src: str, dst: str) -> None:
    cmd = ["rsync", "-av"] + RSYNC_EXCLUDES + [src, dst]
...
def cmd_fetch(source_path: Path) -> None:
    ...
    src_unix = to_unix_path(source_path)
    dst_unix = to_unix_path(PROJECT_DIR)
    rsync(f"{src_unix}/", f"{dst_unix}/")
    logger.info("fetch 完成")


def cmd_send(source_path: Path) -> None:
    """将当前目录 backend/frontend 变更发送到上游"""
    for d in SEND_DIRS:
        src = PROJECT_DIR / d
        dst = source_path / d
        if not src.exists():
            logger.warning(f"跳过不存在的目录: {src}")
            continue
        logger.info(f"send: {src}/ -> {dst}/")
        dst.mkdir(parents=True, exist_ok=True)
        src_unix = to_unix_path(src)
        dst_unix = to_unix_path(dst)
        rsync(f"{src_unix}/", f"{dst_unix}/")
    logger.info("send 完成")
...
def main() -> None:
    if len(sys.argv) < 2 or sys.argv[1] not in ("fetch", "send"):
        print("用法: python scripts/sync_ops.py <fetch|send>")
        sys.exit(1)
