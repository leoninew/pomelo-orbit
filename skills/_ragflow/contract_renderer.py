"""Private shared rendering implementation for topology-specific RAGFlow CLIs."""
...
def generated_header(contract: Mapping[str, Any], renderer_path: str) -> str:
    return (
        f"Generated by {renderer_path} "
...
def render_compose(contract: Mapping[str, Any], target: Mapping[str, Any], renderer_path: str) -> str:
    ...
    header = [
        generated_header(contract, renderer_path),
        "Do not edit this file directly.",
        *contract["compose_header"],
        *target.get("header", []),
    ]
...
def render_primitive(contract: Mapping[str, Any], renderer_path: str) -> str:
    lines = [
        f"# {contract['primitive_title']}\n",
        f"> {generated_header(contract, renderer_path)} Do not edit this file directly.\n",
        "\n",
...
def render_all(contract: Mapping[str, Any], renderer_path: str) -> dict[Path, str]:
    rendered = {
        Path(target["path"]): render_compose(contract, target, renderer_path)
        for target in contract["compose_targets"]
    }
    rendered[Path(contract["primitive_path"])] = render_primitive(contract, renderer_path)
    return rendered
