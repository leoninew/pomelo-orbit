# `export_ragflow_baseline.py` — 导出 RAGFlow SQLite 控制面基线

Doc role: local script reference。与代码冲突时以代码为准。

## 用途

从 SQLite 数据库的只读连接导出 RAGFlow 控制面 SQL 基线。导出范围包含非 Gateway Application、其 Project、Version、组件、Service、Version Endpoint 和 ServiceComponent 映射；Deployment 不在范围内。
...
```bash
# 使用默认数据库，经过完整校验后替换受版本控制的基线
python skills/_ragflow/export_ragflow_baseline.py \
  --output skills/deploy-ragflow-split-orbit/references/ragflow-bundled-sqlite.sql \
  --replace
...
数据库通过 SQLite `mode=ro` 打开；脚本先执行完整性和外键检查。`--output` 必填；已有文件只有明确给出 `--replace` 才会在全部校验成功后原子替换。脚本不会修改数据库。
