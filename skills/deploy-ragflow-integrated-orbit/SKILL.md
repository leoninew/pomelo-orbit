...
2. If the selected cache or image is missing, stop before Orbit writes. Ask for authorization before `prepare-model`, `prepare-image`, or any download. Do not make TEI download `BAAI/bge-m3` at startup or overwrite a non-empty cache.
...
2. Stop `ragflow-split/default` through Orbit with `remove_volumes=false` when it is running, then select the detected profile on `ragflow/default` with `orbit_update_service_basic`. Do not deploy split backing Services in this workflow.
