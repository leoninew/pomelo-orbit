...
5. Before creating the Service or deploying, check whether its `mysql/data` directory already contains MySQL data. If it does, explain that its existing root host rule and password will be retained. Obtain either explicit confirmation to delete the old data and reset MySQL, or the matching existing MySQL credentials from the user. Do not generate a replacement `MYSQL_PASSWORD` while retaining unknown MySQL data.
...
7. Deploy the Version directly without publishing it, then wait for the Deployment terminal state.
...
MySQL initializer variables, including `MYSQL_ROOT_HOST` and `MYSQL_ROOT_PASSWORD`, apply only to an empty `mysql/data` directory. When that directory already exists, it can retain a prior root host rule and password; changing the Service runtime configuration does not update those credentials. A RAGFlow startup error such as `Access denied for user 'root'@'172.*'` can therefore indicate pre-existing MySQL data, not a malformed current runtime configuration.

Before deployment, check for existing `mysql/data` and resolve its credentials before creating a Service. If it contains data, ask the user to either confirm deletion for a clean reset or provide the existing MySQL credentials. Do not silently replace the configured password while retaining that directory.
...
2. With explicit confirmation, manually remove the `mysql/data` directory to reset it. Do not add this recovery action to the Orbit Application API or MCP tool surface. For a full fresh reset, `orbit_delete_application(remove_dir=true)` remains available when the user confirms removal of every managed directory.
