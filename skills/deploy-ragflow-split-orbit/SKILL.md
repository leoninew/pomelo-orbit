...
3. Create or repair `ragflow-split` with exactly two Versions: `ragflow-split-cpu` and `ragflow-split-gpu`. Each contains exactly `tei` and `ragflow-cpu`; the GPU Version changes only the fixed CUDA TEI image and `nvidia/all/gpu` device request. Add the note `GPU runtime unverified` until inference is tested on an NVIDIA host. Use the source types and logical source names in the split primitive: the TEI cache is a directory, while backing data and RAGFlow logs are named volumes. Keep TEI internal; expose `ragflow-cpu:80` only as `gateway_http` through the Gateway `web` entrypoint. Do not add a local or host mapping for RAGFlow.
4. Use the discovered or newly created network hostnames in `ragflow-cpu` environment values. RAGFlow may depend on `tei` locally, but cross-Application backing resources have no Compose `depends_on`; deploy and verify those resources first. Initialize the managed Traefik API as an `http` local endpoint `127.0.0.1:8080 -> 8080`, then verify the Dashboard at `http://127.0.0.1:8080/dashboard/` after Gateway deployment.
