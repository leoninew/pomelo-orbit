...
| --- | --- | --- | --- |
| Integrated RAGFlow + TEI | `ragflow-integrated` | `ragflow-integrated-cpu`, `ragflow-integrated-gpu` | `default` |
...
An existing compatible, healthy backing Service may already be running and does not
need to be stopped merely to satisfy inventory initialization. Preserve its selected
Version and runtime data when it is reused.

## Required Initialization

1. List Projects, Applications, Versions, Services, and active Deployments before any write.
2. Create or repair `ragflow-integrated` with its two six-component Versions and one `default` Service.
3. Create or repair `ragflow-split` with its two two-component Versions and one `default` Service.
4. Create or repair each split backing Application with one complete Version and one stopped `default` Service. Initialize them even when the invoked skill will deploy integrated RAGFlow. This stopped-state requirement applies to newly created Services; preserve an existing compatible, healthy backing Service's running state.
