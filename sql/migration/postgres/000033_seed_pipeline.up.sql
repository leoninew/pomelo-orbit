...
- build', '[{"default":null,"editable":true,"name":"working_dir","secret":false,"source":"pipeline_custom","value":".","description":""}]', 12),
    ('01KZG83K2MXG08EJ6G48SG38B3', '01KRRKK0K3T519ZQZES3M4QA9Z', 'template', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, '镜像构建流水线', '适用于使用 Dockerfile 进制镜像构建的仓库', '[]', 11);
...
# 拉取代码
git fetch --depth=1 --no-tags --force origin {{ repository_ref }}
# 清理未跟踪源码，保留 Git 忽略的依赖缓存
git clean -fd
...
docker build -f {{ repository_dockerfile }} \
  ...
  .', 'Build container image', 3, NULL, NULL, NULL, NULL, '[{"collector":"docker_image","reference":"{{ image_name }}:{{ runtime_datetime }}","name":"{{ image_name }}"}]', NULL, NULL);
