...
export type ComponentFormValidation =
  { valid: true; value: VersionComponentReq } | { valid: false; error: ComponentFormError };
...
function textRows(values: string[]): TextRow[] {
...
export function componentFormFromResponse(component: VersionComponentResp): ComponentForm {
  ...
  return {
    ...
    command: textRows(component.command),
    args: textRows(component.args),
    ...
    mounts: component.mounts.map((item) => ({
      ...
      content_mode: inputText(item.content_mode),
    })),
    networks: textRows(component.networks),
    ...
    healthcheck_test: textRows(healthcheck?.test ?? []),
...
function buildHealthcheck(form: ComponentForm): ComponentHealthcheck | undefined | null {
  ...
  if (!form.healthcheck_disabled) {
    if (
      ...
      form.healthcheck_test.length === 0 ||
      form.healthcheck_test.some((item) => item.value === '')
    ) {
      return null;
    }
  }
  ...
  return {
    ...
    test: form.healthcheck_test.map((item) => item.value),
...
function buildResources(form: ComponentForm): ComponentResources | undefined {
  const resources: ComponentResources = {
    limit_cpus: optionalText(form.resources.limit_cpus),
    limit_memory: optionalText(form.resources.limit_memory),
    reservation_cpus: optionalText(form.resources.reservation_cpus),
    reservation_memory: optionalText(form.resources.reservation_memory),
  };
...
function buildMounts(rows: MountRow[]): ComponentMount[] | null {
  ...
  for (const row of rows) {
    ...
    if (row.source_type === 'file' && row.content_mode === '') {
      return null;
    }
    if (row.source_type !== 'file' && (row.content !== '' || row.content_mode !== '')) {
      return null;
    }
    mounts.push({
      ...
      content_mode: optionalText(row.content_mode),
    });
  }
  return mounts;
}
...
export function componentRequestFromForm(form: ComponentForm): ComponentFormValidation {
  ...
  if (form.command.some((item) => item.value === '')) {
    return { valid: false, error: 'command' };
  }
  if (form.args.some((item) => item.value === '')) {
    return { valid: false, error: 'args' };
  }
  ...
  if (form.networks.some((item) => item.value === '')) {
    return { valid: false, error: 'networks' };
  }
  ...
  const tmpfs = buildTmpfs(form.tmpfs);
  if (tmpfs === null) {
    return { valid: false, error: 'tmpfs' };
  }
  const ulimits = buildUlimits(form.ulimits);
  if (ulimits === null) {
  ...
  return {
    ...
    value: {
      name: form.name,
      image: form.image,
      command: form.command.map((item) => item.value),
      args: form.args.map((item) => item.value),
      env: form.env.map((item) => ({ key: item.key, value: item.value })),
      ports,
      mounts,
      networks: form.networks.map((item) => item.value),
      dependencies: form.dependencies.map((item) => ({
        name: item.name,
        condition: item.condition,
      })),
      healthcheck,
      resources: buildResources(form),
