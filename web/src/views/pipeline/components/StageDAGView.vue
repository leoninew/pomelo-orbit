...
<script setup lang="ts">
  ...
  import type { SnapshotStageResp } from '@/gen/proto/orbit/v1/pipeline/snapshot';
  ...
  const emit = defineEmits<(e: 'view-stage', stageRun: PipelineStageRunResp) => void>();
  ...
  function syncStatus() {
    const map = new Map<string, PipelineStageRunResp>();
    for (const sr of props.stageRuns ?? []) {
  ...
  function handleNodeClick(event: { node: Node }) {
    const sr = (event.node.data as { stageRun?: PipelineStageRunResp }).stageRun;
    if (sr) {
      emit('view-stage', sr);
    }
  }
